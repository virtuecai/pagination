...
!function ($) {
    ...
    var Page = function (element, options) {
        var defaultOption = {
            ...
            showPageSizes: false,
            pageSizeItems: null,
            pageElementSort: ['$info', '$size', '$page', '$jump'],//可选, 显示的先后顺序, $info:"1 ~ 5条，共44条",  '$size': pageSize, '$page', 'pageNumber', '$jump':指定页面跳转
            debug: false
        }
        this.$element = $(element);
        this.$page = $('<ul class="m-pagination-page pagination"></ul>');
        this.$size = $('<div class="m-pagination-size"></div>');
        this.$jump = $('<div class="m-pagination-jump input-group"></div>');
        ...
        var init = function (obj) {
            ...
            //init size module
            var html = $('<select class="form-control" data-page-btn="size"></select>');
            ...
            //init jump module
            var jumpHtml = '<input type="text" class="form-control text-center"><span class="input-group-btn"><button data-page-btn="jump" class="btn btn-default" type="button">' + that.options.jumpBtnText + '</button></span>';
            ...
            //update by virtuecai
            if(that.options.pageElementSort && that.options.pageElementSort.length > 0) {
                $.each(that.options.pageElementSort, function () {
                    that.$element.append(that[this].hide());
                });
            } else {
